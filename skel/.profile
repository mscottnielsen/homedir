...
[ -f $HOME/.h_shell ] && RUN_SUB_SHELL=$(cat $HOME/.h_shell)  # if no /bin/bash (z/OS)
...
h_tty="tty$(expr $(echo ${h_tty:-x}) : '[^0-9]*\([0-9]*\)')" # bash: h_tty="tty${h_tty//[^0-9]/}"
...
h_log() {
  ...
  printf "$pref (+$(($(h_tstamp) - ts_start0))s) $*" >> $HOMEDIR_LOG
  return 0
}
...
RUN_SUB_SHELL_OK=99
[ "$RUN_SUB_SHELL" != "" ] \
  && printf -- "$0" | egrep -v $(basename  $(echo "$RUN_SUB_SHELL" | cut -f1)) >/dev/null 2>&1 \
  && echo "Default shell is $0 => now logging into \"$RUN_SUB_SHELL\"...." \
  && h_log "launching \"$RUN_SUB_SHELL\"....\n" \
  && $RUN_SUB_SHELL \
  && RUN_SUB_SHELL_OK=$? \
  && [ "$RUN_SUB_SHELL_OK" != "99" ] \
  && exit $RUN_SUB_SHELL_OK
